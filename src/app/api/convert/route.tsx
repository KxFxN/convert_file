    ...
    const parsedData = Papa.parse<DataType>(text, {
      ...
      transform: (value, field) => {
        if (typeof value === 'string') {
          ...
          if (['38', '39'].includes(field as string) && value.match(/^\d+\.\d+$/)) {
...
function convertToXML(data: DataType[], selected: string) {
  ...
  const findFirstNonEmptyValue = (data: DataType[], columnKey: string): string => {
    const value = data.find((row) => row[columnKey] && String(row[columnKey]).trim() !== '')?.[
      columnKey
    ];
  ...
  const result = fields.reduce((acc: Record<string, string>, { key, name, transform }) => {
  ...
  const mandatoryItems = ['Purity', 'Other Fluorocarbons', 'Acid content(as HF)', 'H2O', 'SF6'];
  ...
  data.forEach((row) => {
    ...
    if (mandatoryItems.includes(itemName)) {
      ...
      const specSymbol = rawSpecSymbol === '' ? defaultspecSymbol[itemName] : rawSpecSymbol;

      // ตรวจสอบ specValue: ใช้ row["39"] หรือ row["38"] แล้ว trim ถ้าว่างให้ใช้ defaultSpec
      const rawSpecValue = (String(row['39'] || '') || String(row['38'] || '')).trim();
      const specValue = rawSpecValue === '' ? defaultSpec[selected][itemName] : rawSpecValue;
      ...
      const detectionLimitValue =
        rawDetectionLimit === '' ? defaultDetect[selected][itemName] : rawDetectionLimit;
      const detectionLimit = String(detectionLimitValue).replace(/<=|>=|>|</g, '');
  ...
  const materialNumber = selected === '14EH' ? '130-02004' : selected === '32EH' ? '130-05393' : '';
    ...
    Micron_COA: {
        ...
        BasicInfoField: [
            ...
            $: {
              ...
              FieldValue:
                selected === '14EH'
                  ...
                  : selected === '32EH' && 'ZN5YQVW54AFP-18-20951',
            },
          },
          {
            $: {
              FieldName: 'Version',
              FieldValue: selected === '14EH' ? '2' : selected === '32EH' ? '1' : '1',
            },
          },
          ...
          { $: { FieldName: 'Materialtype', FieldValue: 'Gas' } },
          {
            $: {
              ...
              FieldValue: selected === '14EH' ? 'CF4 30*15' : selected === '32EH' && 'CH2F2 47L',
            },
          },
          {
            $: {
              FieldName: 'SupplierPartNo',
              FieldValue: selected === '14EH' ? selected : selected === '32EH' && 'HFC32EH',
            },
          },
          {
            $: {
              FieldName: 'MaterialNumber',
              FieldValue: selected === '14EH' ? '130-02004' : selected === '32EH' && '130-05393',
            },
          },
          ...
          { $: { FieldName: 'LotQuantity', FieldValue: result.LotQuantity } },
          {
            $: {
              ...
              FieldValue: selected === '14EH' ? 'Bundled' : selected === '32EH' && 'Cylinder',
            },
          },
      ...
      Content: {
        UnitId: Array.from(sortedMap.entries()).map(([key, value]) => ({
          ...
          InspectionItem: Array.from(value.entries()).map(([itemName, details]) => ({
            $: {
